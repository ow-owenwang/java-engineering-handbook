...
import org.example.upload.beans.FileUploadBean;
import org.example.upload.db.UploadFileDao;
import org.example.upload.exception.InvalidExtNameException;
...
public class FileUploadServlet extends HttpServlet {
    ...
    private List<FileUploadBean> buildFileUploadBeans(List<FileItem> items, Map<String, FileItem> uploadFiles) throws UnsupportedEncodingException {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (!item.isFormField()) {
                ...
                String desc = descMap.get(descName);
    ...
    private ServletFileUpload getServletFileUpload() {
        String fileMaxSize = UploadUtils.getMap().get("file.max.size");
        String totalFileMaxSize = UploadUtils.getMap().get("total.file.max.size");
